...
class AutoencoderKL(pl.LightningModule):
    def __init__(self,
        ...
        if ckpt_path is not None:
            self.init_from_ckpt(ckpt_path, ignore_keys=ignore_keys)
        self.random_sample_box = [1, 2, 3, 4, 5]
    ...
    def decode(self, z, random_idx=None): # random_idx -> [1, 2, 3, 4, 5] = T1, T2, FL, WB, BB
        if random_idx is not None: # when b, 12, 64, 64 is input, make it b, 4, 64, 64.
            ...
            style_latent = z[:, random_idx*2:random_idx*2+2, :, :]
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        b = inputs.shape[0]
        three_random_idx = random.sample(self.random_sample_box, 3) # random_sample_box = [1, 2, 3, 4, 5] meaning [T1, T2, FL, WB, BB] respectively
        for i, idx in enumerate(three_random_idx): # ex. 1, 3, 4 = T1, FL, WB
            if i == 0:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # LOSS is calculated with random 3 sequence averaged
        b = inputs.shape[0]
        three_random_idx = random.sample(self.random_sample_box, 3) # random_sample_box = [1, 2, 3, 4, 5] = T1, T2, FL, WB, BB
        for i, idx in enumerate(three_random_idx): # ex. 1, 3, 4 = T1, FL, WB
            if i == 0:
                reconstructions = self.decode(z, idx) # if idx is 3, decode z with [5, 6] channel -> 6, 1, 256, 256
                single_inputs = inputs[:, idx-1, :, :].unsqueeze(1) # if idx is 3, get corresponding flair -> 6, 1, 256, 256
    ...
    def log_images(self, batch, only_inputs=False, **kwargs):
        ...
        if not only_inputs:
            ...
            if x.shape[1] > 3 and x_batch_0.shape[1] > 3:
                x = self.to_rgb(x)
                x_batch_0 = self.to_rgb(x_batch_0)
                x_batch_1 = self.to_rgb(x_batch_1)
                ...
                xrec = self.to_rgb(xrec) # b, 1, 256, 256
                xrec_0_0 = self.to_rgb(xrec_0_0) # 1, 1, 256, 256
                xrec_1_0 = self.to_rgb(xrec_1_0)
                xrec_2_0 = self.to_rgb(xrec_2_0)
                xrec_3_0 = self.to_rgb(xrec_3_0)
                xrec_4_0 = self.to_rgb(xrec_4_0)

                xrec_0_1 = self.to_rgb(xrec_0_1) # 1, 1, 256, 256
                xrec_1_1 = self.to_rgb(xrec_1_1)
                xrec_2_1 = self.to_rgb(xrec_2_1)
                xrec_3_1 = self.to_rgb(xrec_3_1)
                xrec_4_1 = self.to_rgb(xrec_4_1)

                xrec_0_mix = self.to_rgb(xrec_0_mix) # 1, 1, 256, 256
                xrec_1_mix = self.to_rgb(xrec_1_mix)
                xrec_2_mix = self.to_rgb(xrec_2_mix)
                xrec_3_mix = self.to_rgb(xrec_3_mix)
                xrec_4_mix = self.to_rgb(xrec_4_mix)
